	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
  	...
  	//STEP 2: Register JDBC driver
  		try{
  			...
  			String sql = "SELECT distinct fileLocation, title, author, ISBN, shelfmark, available from books where ";
  			System.out.println(searchOn);
  			if(searchOn.equals("keyword") || searchOn.equals("title")){
  				sql = sql + "title like \'%" + searchVal + "%\' or author like \'%" + searchVal + "%\' or genre like \'%" + searchVal + "%\'";
 			 ...
 			 while(rs.next()){
 				 ...
 				 String sm = rs.getString("shelfmark");
 				 ...
 				 String html = "<div class=\"grid-item\" style=\"width: 80%; margin:auto;cursor: pointer; padding-bottom: 5%;\">"
 						+ "<form action=\"http://3.145.193.254:8080/LibraryBR/ViewBook\" method=\"post\" style=\"width: 60%; margin: auto;\">"
 						+ "<input type=\"hidden\" name=\"searching\" value=\"" + sm + "\"><button type=\"submit\">"
 				 		+ "<img src=\"" + l + "\" style=\"width: 60%\"></button></form><h3>" + t + "</h3> <h5>" + au + "</h5>"
// 				 				+ "<form action=\"http://3.145.193.254:8080/LibraryBR/ViewBook\" method=\"post\" style=\"width: 60%; margin: auto;\">"
 				 				// + "<input type=\"hidden\" name=\"searching\" value=\"" + num + "\"><input type=\"submit\" value=\"More Info\"></form>"
 				 		+ "</div>";
 				 s.append(html);
