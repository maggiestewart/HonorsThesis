...
public class CardLogin extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cardNo == null){
			cardNo = "21659156097940";
		}
		String pass = request.getParameter("accountPW");
		...
		if(!Validate.checkUser(cardNo, pass))
        {
			s.append(htmlInfo);
            s.append("<h3 style=\"text-align: center; padding-top: 5%; padding-bottom: 5%;\">Username or Password incorrect.</h3>");
            s.append(footer);
            out.println(s);
		...
		session.setAttribute("card", cardNo);
		//setting session to expire in 30 minutes
		session.setMaxInactiveInterval(30*60);
		Cookie card = new Cookie("card", cardNo);
		card.setMaxAge(30*60);
		response.addCookie(card);
		
		card.setDomain("example.com");
		card.setPath("http://3.145.193.254:8080/LibraryBR/");
		...
		Cookie[] cookies = request.getCookies();
		  if(cookies != null) {
		      for (int i = 0; i < cookies.length; i++) {
		          card=cookies[i];
		          cookieName = card.getName();
		          cookieValue = card.getValue();
		       }
		   }
		...
		*/
        ...
        Cookie ck = new Cookie("cardNo", cardNo);
        response.addCookie(ck);
        ck.setMaxAge(1800); // 30 minutes
